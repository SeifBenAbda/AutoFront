...
const ClientGender: React.FC<ClientGenderProps> = ({ option1, option2, onChange ,defaultValue}) => {
  ...
  return (
    <Toggle
      ...
      className={`relative w-full font-oswald bg-whiteSecond text-highGrey2 hover:bg-whiteSecond hover:text-highGrey2 border 
      data-[state=on]:border-lightRed data-[state=on]:bg-lightRed data-[state=on]:text-whiteSecond  
      ${selected === "Madame" ? "border-lightRed bg-lightRed text-whiteSecond" : "border-whiteSecond bg-whiteSecond text-highGrey2"}`}
    >
      {selected}
    </Toggle>
  );
