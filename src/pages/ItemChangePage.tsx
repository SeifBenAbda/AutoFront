...
import {
  defaultFormClient,
  defaultGeneralItemChnageForm,
  defaultItemRequestList,
  defaultRappelList,
...
const ItemChangePage: React.FC = () => {
  ...
  const [selectedClient, setSelectedClient] = useState<any>(undefined); // Selected client state
  const { user } = useUser();
  ...
  const handleSelectClient = (client: any) => {
    ...
    form.reset({
      ...currentValues,
      clientForm: parsedClient,
    });
  };

  // Ensure modal data is fetched when opened
  useEffect(() => {
  ...
  return (
    <div className="relative">
        ...
        <div className="flex flex-col">
          <CardHeader className="ml-4 mr-4 flex flex-col md:flex-row md:items-center md:justify-between fixed top-[60px] left-0 right-0 bg-veryGrey z-10 p-4 border-b border-veryGrey">
            <div className="flex-1">
              <CardTitle className="text-highGrey">Devis</CardTitle>
            ...
            <div className="flex flex-row space-x-2 mt-2 md:mt-0 md:flex-1 md:justify-end">
              <Button
                ...
                type="button"
                disabled={isLoading}
                className="bg-lightWhite hover:bg-lightWhite border border-highGrey text-highGrey"
              >
                Client existant
              </Button>
              <Button
      ...
      <ClientModal
        ...
        selectedClient={selectedClient} // Pass selected client
      />
    </div>
  );
};
